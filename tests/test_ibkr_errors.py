...
def test_redact_short_token_only_scrubs_query_param():
    ...
    assert _redact(msg, "0") == (
        "SendRequest?v=3&t=<redacted>&q=999 (status 403 Forbidden)"
    )
...
def test_extract_1012_raises_config_error(importer, ibkr_yaml, monkeypatch):
    ...
    monkeypatch.setattr(
        "beancount_tools_collection.importers.ibkr.client.download", boom
    )
...
def test_bad_response_traceback_excludes_body_and_token(
    importer, ibkr_yaml, monkeypatch
):
    token = "SECRETTOKEN123"
    response = MagicMock()
...
def test_keyboard_interrupt_during_config_load_propagates(
    importer, ibkr_yaml, monkeypatch
):
    ...
    monkeypatch.setattr(
        "beancount_tools_collection.importers.ibkr.yaml.safe_load", interrupt
    )
...
def test_exceptions_trap_prints_bare_message_without_traceback():
    ...
    with trap:
        raise IBKRConfigError(
            "IBKR import failed: the IBKR Flex token has expired (code 1012)"
        )
