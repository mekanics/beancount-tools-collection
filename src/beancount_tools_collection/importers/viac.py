...
class ViacImporter(Importer):
    ...
    def __init__(
        ...
        self.account_map = {
            ...
            "S3a": {"Portfolio": "Assets:Vorsorge:S3a:Viac:Portfolio{}"},
        }
    ...
    def _get_s2_account_type(self, source_account=None):
        ...
        if source_account and source_account.endswith(".O"):
            ...
            return (
                "Obligatorium"
                if "Obligatorium" in self.main_account
                else "Ueberobligatorium"
            )
    ...
    def fix_accounts(self, filepath):
        ...
        if pillar == "S2":
            ...
            self.main_account = (
                self._get_mapped_account(pillar, "Obligatorium") or self.root_account
            )
        elif pillar == "S3a" and portfolio.startswith("Portfolio"):
            portfolio_num = portfolio[9:]  # Extract number from "PortfolioX"
            self.main_account = (
                self._get_mapped_account(pillar, "Portfolio", portfolio_num)
                or self.root_account
            )
        ...
        if not self.main_account:
            ...
            self.main_account = re.sub(
                r"Portfolio\d|Freizuegigkeit|Ueberobligatorium", portfolio, new_account
            )
    ...
    def extract(self, filepath, existing=None):
        ...
        for account_key in [
            k for k in transactions.keys() if not k.endswith(("D1", "D2"))
        ]:
            ...
            if self.main_account.startswith("Assets:Vorsorge:S2"):
                if account_key.endswith(".O"):
                    self.main_account = self._get_mapped_account(
                        "S2", "Obligatorium"
                    ) or self.main_account.replace("Freizuegigkeit", "Obligatorium")
                elif account_key.endswith(".U"):
                    self.main_account = self._get_mapped_account(
                        "S2", "Ueberobligatorium"
                    ) or self.main_account.replace(
                        "Freizuegigkeit", "Ueberobligatorium"
                    )
    ...
    def Trades(self, trades):
        ...
        for idx, row in trades.iterrows():
            ...
            if document is not None and document != "nan":
    ...
    def Fees(self, fees):
        ...
        for idx, row in fees.iterrows():
            ...
            postings = [
                data.Posting(
                    self.getFeesAccount(currency, row["source_account"]),
                    -amount_,
                    None,
                    None,
                    None,
    ...
    def Dividends(self, dividends):
        ...
        for _idx, row in dividends.iterrows():
            ...
            metadict = {"isin": share["isin"], "source_account": row["source_account"]}
    ...
    def Balances(self, df):
        ...
        meta = data.new_metadata(
            "balance", 0, {"source_account": transaction["source_account"]}
        )
