...
class ViacImporter(Importer):
    ...
    def identify(self, filepath):
        ...
        logger.info(f"identify assertion for viac importer and file '{filepath}': {result}")
        return result
    ...
    def fix_accounts(self, filepath):
        ...
        if pillar == 'S2':
            ...
            self.main_account = (
                self._get_mapped_account(pillar, 'Portfolio', portfolio_num) or self.root_account
            )
        ...
        for account_key in [k for k in transactions.keys() if not k.endswith(('D1', 'D2'))]:
            ...
            if self.main_account.startswith('Assets:Vorsorge:S2'):
                if account_key.endswith('.O'):
                    ...
                    ) or self.main_account.replace('Freizuegigkeit', 'Ueberobligatorium')
