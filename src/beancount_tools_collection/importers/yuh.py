...
class YuhImporter(Importer):
    ...
    def identify(self, filepath):
        ...
        logger.info(
            f"identify assertion for yuh importer and file '{filepath}': {result}"
        )
        return result
    ...
    def _clean_payee(self, activity_name, activity_type):
        ...
        if activity_type in ["PAYMENT_TRANSACTION_IN", "PAYMENT_TRANSACTION_OUT"]:
            payee = (
                payee.replace("Transfer from ", "")
                .replace("Transfer to ", "")
                .replace("Überweisung von ", "")
                .replace("Überweisung an ", "")
            )

        # Clean up Twint transactions and use title case
        if "twint" in payee.lower():
            payee = (
                payee.replace("Twint from ", "")
                .replace("Twint to ", "")
                .replace("Twint von ", "")
                .replace("Twint an ", "")
                .title()
        ...
        if activity_type in ["PAYMENT_TRANSACTION_IN", "PAYMENT_TRANSACTION_OUT"] and (
            "standing order" in payee.lower() or "dauerauftrag" in payee.lower()
        ):
    ...
    def extract(self, filepath, existing=None):
        ...
        foreign_mask = (
            (df["DEBIT_ABS"] > 0)
            & (df["DEBIT CURRENCY"] != "CHF")
            & (df["DEBIT CURRENCY"].notna())
        )
        ...
        if not auto_exchanges.empty and not foreign_txns.empty:
            ...
            matched_foreign_indices = set(
                matched["_orig_idx_orig"].dropna().astype(int)
            )
            matched_auto_indices = set(
                matched[matched["_orig_idx_orig"].notna()]["_orig_idx"].astype(int)
            )
        else:
        ...
        unmatched_auto = auto_exchanges[
            ~auto_exchanges["_orig_idx"].isin(matched_auto_indices)
        ]
    ...
    def _create_combined_transaction(self, filepath, row):
        ...
        try:
            ...
            orig_date = (
                row["DATE_orig"].date()
                if pd.notna(row["DATE_orig"])
                else row["DATE"].date()
            )

            payee, narration, tags = self._clean_payee(
                row["ACTIVITY NAME_orig"], row["ACTIVITY TYPE_orig"]
            )
            ...
            logger.info(
                f"Created combined transaction for {payee} with original amount {credit_amount} {credit_currency}"
            )
            return txn
    ...
    def _create_goal_transaction(self, filepath, row):
        ...
        try:
            ...
            logger.info(
                f"Processing {'deposit to' if is_deposit else 'withdrawal from'} goal: {goal_name}"
            )
            ...
            txn = data.Transaction(
                ...
                postings=[
                    data.Posting(
                        self.main_account,
                        amount.Amount(
                            -amount_num if is_deposit else amount_num, currency
                        ),
                        None,
                        None,
                        None,
    ...
    def _create_regular_transaction(self, filepath, row):
        ...
        try:
            ...
            payee, narration, tags = self._clean_payee(
                row["ACTIVITY NAME"], row["ACTIVITY TYPE"]
            )
